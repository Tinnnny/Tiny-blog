...
---
# Pagehelper和Tk.mybatis

## 概述
Tk.mybatis 是在MyBatis框架的基础上提供了很多工具，让开发更加高效。是一个国人开发的工具框架，封装了很多数据库相关的方法。

PageHelper 是Mybatis的分页插件，支持多数据库、多数据源。可以简化数据库的分页查询操作，整合过程也极其简单，只需引入依赖即可。
## 起步
::: danger 使用Pagehelper和Tk.mybatis分几步：
1. 引入`mapper-spring-boot-starter`和`pagehelper-spring-boot-starter`依赖。
2. 在`application.yml `中配置数据库连接,在 `generatorConfig.xml` 配置自动生成。
3. 创建通用的父级接口。
4. 在 `pom.xml`文件中增加 `mybatis-generator-maven-plugin`插件。
:::
### 1.引入依赖
在pom文件中引入MyBatis的依赖
```xml
...
```
在 `src/main/resources/generator/ `目录下创建 `generatorConfig.xml` 配置自动生成：
```xml
...
```
值得注意的是，虽然已经配置了druid的连接，但是这里还是需要配置数据源，不然会发生错误，也就是说，这里有两个配置文件。即`jdbc.properties`和`application.yml`。两个文件皆在src/main/resources 目录下创建。
...
#### 注意
直接使用tk.mybatis封装的方法如insert插入数据时会出现语法错误。

<div align="center">
<img src="http://ww1.sinaimg.cn/large/af2b2d1bly1g3kho79atnj215905u44q.jpg">
</div>

究其原因是在`mybatis-generator-maven-plugin`插件自动生成的代码上有一个`@Table`注解，注解上除了表名还写了数据库。删掉数据库名即可正确使用方法。
...
其中，在更新数据时如果使用`updateByExample(tbSysUser, example)`方法其实不太好，因为需要填写要求的not null的字段。而updateByExampleSelective(tbSysUser, example)方法可以只填写需要更新的数据则可。
